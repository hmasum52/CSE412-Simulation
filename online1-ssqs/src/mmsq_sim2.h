...
void MMSQSim::initialize() {
  ...
  num_in_q = 0;
  time_last_event.resize(num_of_server, -1);
...
void MMSQSim::arrive() {
  ...
  if (free_server_index == -1) {
    ...
    if (time_last_event[free_server_index] == -1) {
      time_last_event[free_server_index] = sim_time;
    }
  }
}
...
//  utilization will be counted
  for (int i = 0; i < num_of_server; i++) {
    if (server_status[i] == ServerStatus::BUSY) {
      time_since_last_event = sim_time - time_last_event[i];
      time_last_event[i] = sim_time;
