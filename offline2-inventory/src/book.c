...
main() /* Main function. */
{
    ...
    for (i = 1; i <= num_policies; ++i)
    {
        ...
        do
        {
            ...
            // fprintf(outfile, "time: %f, next_event_type: %d\n",sim_time, next_event_type);
            switch (next_event_type)
            {
            case 1:
                order_arrival();
                //fprintf(outfile, "order: inv_level: %d\n", inv_level);
                break;
            case 2:
                demand();
                //fprintf(outfile, "demand: inv_level: %d\n", inv_level);
                break;
            case 4:
                evaluate();
                //fprintf(outfile, "evaluate: inv_level: %d\n", inv_level);
                break;
            case 3:
                report();
                break;
...
void demand(void) /* Demand event function. */
{
    ...
    int r = random_integer(prob_distrib_demand);
   // fprintf(outfile, "demand: random: %d\n", r);
    inv_level -= r;
...
void report(void) /* Report generator function. */
{
    ...
    fprintf(outfile, "\n\n(%3d,%3d)%15.2f%15.2f%15.2f%15.2f",
            ...
            avg_ordering_cost, avg_holding_cost, avg_shortage_cost);
    fprintf(outfile, "'\n%15.2f%15.2f%15.2f",
            total_ordering_cost, area_holding, area_shortage);
}
